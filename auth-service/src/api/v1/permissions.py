...
from services.permissions import PermissionService, get_permission_service
...
async def create_permission(
    permission_create: Annotated[
        PermissionCreate, Body(description="Шаблон для создания привелегии")
    ],
    ...
    if await permission_service.check_permission_exists(
        permission_create_encoded["permission_name"]
    ):
        raise HTTPException(
    ...
    return await permission_service.add_permission(permission_create_encoded)
...
async def update_permission(
    permission_id: Annotated[
        UUID, Path(description="Идентификатор привелегии")
    ],
    permission_upate: Annotated[
        PermissionUpdate, Body(description="Шаблон для обновления привелегии")
    ],
    permission_service: Annotated[
        PermissionService, Depends(get_permission_service)
    ],
    ...
    if await permission_service.check_permission_name_duplicates(
        permission_id, permission_update_encoded["permission_name"]
    ):
        raise HTTPException(
            status_code=HTTPStatus.CONFLICT,
            detail="Permission with this name already exists",
    ...
    if not permission:
        raise HTTPException(
            status_code=HTTPStatus.NOT_FOUND, detail="Permisson not found"
        )
