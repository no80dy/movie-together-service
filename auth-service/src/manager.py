...
from models.entity import Group, Permission, User
...
async def check_permission_name_exists(
...
async def check_group_name_exists(
    group_name: str, session: AsyncSession
) -> Group:
    group = (
        (
            await session.execute(
...
async def create_superuser():
    ...
    async with async_session() as session:
        async with session.begin():
            if not await check_permission_name_exists(
                SUPERUSER_PERMISSION_NAME, session
            ):
                permission = Permission(SUPERUSER_PERMISSION_NAME)
                session.add(permission)
            group = await check_group_name_exists(
                SUPERUSER_GROUP_NAME, session
            )
            if not group:
                group = Group(
