...
async def create_fake_tokens():
    async def inner(user_id: str, username: str) -> dict:
        ...
        fake_access_token = await authorize.create_access_token(
            subject=username, user_claims=user_claims
        )
        fake_decrypted_access_token = await authorize.get_raw_jwt(
            fake_access_token
        )
        fake_refresh_token = await authorize.create_refresh_token(
            subject=username, user_claims=user_claims
        )
        fake_decrypted_refresh_token = await authorize.get_raw_jwt(
            fake_refresh_token
        )
...
async def create_fake_login(
    ...
    async def inner() -> dict:
        ...
        fake_tokens = await create_fake_tokens(
            str(fake_user.id), fake_user.username
        )
        fake_user_session = RefreshSession(
            user_id=fake_user.id,
            refresh_jti=fake_tokens["decrypted_refresh_token"]["jti"],
            expired_at=datetime.fromtimestamp(
                fake_tokens["decrypted_refresh_token"]["exp"]
            ),
            user_agent=fake_user_agent,
            is_active=True,
        )
