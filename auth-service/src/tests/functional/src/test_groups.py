...
async def test_create_group(
    create_superuser, make_post_request, group_create, expected_response
):
    ...
    result = await make_post_request(
        "groups/", group_create, {"Authorization": f"Bearer {access_token}"}
    )

    assert result["status"] == expected_response["status"]
    assert (
        result["body"]["group_name"] == expected_response["body"]["group_name"]
    )
    assert (
        result["body"]["permissions"]
        == expected_response["body"]["permissions"]
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ({"group_name": None, "permissions": []}, {"status": HTTP_422}),
        ({"group_name": 123, "permissions": []}, {"status": HTTP_422}),
    ],
)
...
async def test_read_groups(
    ...
    for group in group_data["groups"]:
        await make_post_request(
            "groups/", group, {"Authorization": f"Bearer {access_token}"}
        )

    result = await make_get_request(
        "groups/", {}, {"Authorization": f"Bearer {access_token}"}
    )
...
async def test_update_group(
    ...
    group = await make_post_request(
        "groups/", group_update, {"Authorization": f"Bearer {access_token}"}
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ({"group_name": None, "permissions": []}, {"status": HTTP_422}),
        ({"group_name": 12345, "permissions": []}, {"status": HTTP_422}),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        (
            {"group_name": "new_group", "permissions": []},
            {"status": HTTP_200, "content": "deleted successfully"},
        )
    ],
)
async def test_delete_group(
    ...
    group = await make_post_request(
        "groups/", group_delete, {"Authorization": f"Bearer {access_token}"}
    )

    group_id = group["body"]["id"]
    result = await make_delete_request(
        f"groups/{group_id}", {"Authorization": f"Bearer {access_token}"}
    )
