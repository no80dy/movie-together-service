...
async def test_create_permission(
    create_superuser, make_post_request, permission_create, expected_response
):
    ...
    assert (
        result["body"]["permission_name"]
        == expected_response["body"]["permission_name"]
    )


@pytest.mark.parametrize(
    "permission_create, expected_response",
    [({"permission_name": "new_permission"}, {"status": HTTP_409})],
)
...
@pytest.mark.parametrize(
    "permission_create, expected_response",
    [
        ({"permission_name": None}, {"status": HTTP_422}),
        ({"permission_name": 1234}, {"status": HTTP_422}),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ({"permissions_count": 20}, {"status": 200, "length": 21}),
        ({"permissions_count": 10}, {"status": 200, "length": 11}),
    ],
)
async def test_permissions_read(
    ...
    result = await make_get_request(
        "permissions/", {}, {"Authorization": f"Bearer {access_token}"}
    )
...
@pytest.mark.parametrize(
    ...
    [
        ({"permission_name": None}, {"status": HTTP_422}),
        ({"permission_name": 12345}, {"status": HTTP_422}),
    ],
)
...
@pytest.mark.parametrize(
    "permission_update, expected_response",
    [({"permission_name": "new_permission"}, {"status": HTTP_404})],
)
...
@pytest.mark.parametrize("expected_response", [({"status": HTTP_404})])
async def test_permission_delete_does_not_exists(
    create_superuser, make_post_request, make_delete_request, expected_response
):
    await create_superuser("superuser", "password123")
    result = await make_post_request(
        "users/signin", {"username": "superuser", "password": "password123"}
