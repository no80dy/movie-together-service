...
async def add_group(
    ...
    group_assign: Annotated[
        GroupAssign, Body(description="Шаблон создания роли для пользователя")
    ],
    ...
    if not user:
        raise HTTPException(
            status_code=HTTPStatus.NOT_FOUND, detail="role or user not found"
        )
    return user
...
async def delete_group(
    user_id: Annotated[UUID, Path(description="Идентификатор пользователя")],
    group_assign: Annotated[
        GroupAssign, Body(description="Шаблон удаления роли для пользователя")
    ],
    user_service: Annotated[
        UserPermissionsService, Depends(get_user_permissions_service)
    ],
    ...
    group_assign_encoded = jsonable_encoder(group_assign)
    user = await user_service.delete_role_from_user(
        user_id, group_assign_encoded
    )
    if not user:
        raise HTTPException(
            status_code=HTTPStatus.NOT_FOUND, detail="role or user not found"
...
@router.post(
    "/signup", response_model=UserInDB, status_code=HTTPStatus.CREATED
)
async def create_user(
    ...
    repeated_pass_true = await user_service.check_repeated_password(
        user_dto.get("password"), user_dto.get("password")
    )

    user_exist = await user_service.check_exist_user(user_dto)
    if not repeated_pass_true or user_exist:
        raise HTTPException(
            status_code=400, detail="Некорректное имя пользователя или пароль"
        )
...
async def change_password(
    ...
    if updated_user:
        ...
        raise HTTPException(
            status_code=400, detail="Введены некорректные данные"
        )
...
async def login(
    ...
    active_user_login = await user_service.check_if_user_login(
        str(user.id), user_agent
    )
    if active_user_login:
        # Check if refresh token was expired
        if await user_service.check_unexpected_refresh_token_freshness(
            str(user.id), user_agent
        ):
            await user_service.put_logout_history_in_db(
                str(user.id), user_agent
            )
            await user_service.del_refresh_session_in_db(
                str(user.id), user_agent
            )
        else:
            return JSONResponse(
    ...
    user_claims = {
        "user_id": str(user.id),
        "groups_permissions": await user_service.get_user_groups_permissions(
            user.id
        ),
    }
    ...
    await user_service.put_refresh_session_in_db(
        str(user.id), user_agent, decrypted_token
    )
...
async def refresh(
    ...
    # удаляем сессию из таблицы refresh_sessions
    session_exist = await user_service.check_if_session_exist(
        user_id, user_agent
    )
    if session_exist:
        await user_service.del_refresh_session_in_db(user_id, user_agent)
    ...
    await user_service.put_refresh_session_in_db(
        user_id, user_agent, new_decrypted_token
    )
...
async def get_history(
    ...
    previous, next_page = await user_service.calc_previous_and_next_pages(
        page_number, page_size, count
    )

    result = {"previous": previous, "next": next_page, "items": history}
...
async def auth_via_yandex(
    ...
    if not user:
        ...
        user = await user_service.get_user_by_email(
            social_user["default_email"]
        )
        if user:
            await user_service.add_user_social(
                user.id, social_name, social_user
            )
        else:
            user = await user_service.create_user_social(
                social_name, social_user
            )
            await user_service.add_user_social(
                user.id, social_name, social_user
            )
...
async def remove_social_account(
    ...
    return JSONResponse(
        status_code=HTTPStatus.OK,
        content={
            "detail": f"Аккаунт {social_name} откреплен от личного кабинета успешно"
        },
    )
