...
class Group(Base):
    ...
    permissions = relationship(
        "Permission", secondary=groups_permissions_table, lazy="joined"
    )
...
class User(Base):
    ...
    user_login_history = relationship(
        "UserLoginHistory", cascade="all, delete"
    )
    user_social_networks = relationship(
        "UserSocialNetwork", lazy="selectin", cascade="all, delete"
    )

    groups = relationship("Group", secondary=groups_users_table, lazy="joined")
...
class UserLoginHistory(Base):
    ...
    def __init__(
        self, user_id: UUID, user_agent: str, logout_at: datetime | None = None
    ) -> None:
        self.user_id = user_id
        self.user_agent = user_agent
