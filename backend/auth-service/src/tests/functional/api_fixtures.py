...
def make_get_request(fastapi_session: aiohttp.ClientSession):
    async def inner(
        endpoint: str, query_data: dict, headers: dict | None = None
    ):
        url = test_settings.SERVICE_URL + f"/api/v1/{endpoint}"
        async with fastapi_session.get(
            url, params=query_data, headers=headers
        ) as response:
            body = (
                await response.json()
            ...
            response = {"body": body, "headers": headers, "status": status}
            return response
...
def make_post_request(fastapi_session: aiohttp.ClientSession):
    async def inner(
        ...
        url = test_settings.SERVICE_URL + f"/api/v1/{endpoint}"
        async with fastapi_session.post(
...
def make_put_request(fastapi_session: aiohttp.ClientSession):
    async def inner(endpoint: str, body: dict, headers: dict | None = None):
        url = test_settings.SERVICE_URL + f"/api/v1/{endpoint}"
        async with fastapi_session.put(
            url, json=body, headers=headers
        ) as response:
            body = (
                await response.json()
...
def make_delete_request(fastapi_session: aiohttp.ClientSession):
    async def inner(endpoint: str, headers: dict | None = None):
        url = test_settings.SERVICE_URL + f"/api/v1/{endpoint}"
        async with fastapi_session.delete(url, headers=headers) as response:
            body = (
                await response.json()
                if response.headers["Content-type"] == "application/json"
                else response.text()
