...
class ClientIDService:
    ...
    def make_client_id(
        ...
        return ClientIDService._encrypt(bytes(client_id_raw, 'utf-8'), settings.client_id_key).decode('utf-8')

    @staticmethod
    def get_film_id_from_client_id(
            client_id: str,
    ) -> str:
        client_id_raw = ClientIDService._decrypt(bytes(client_id, 'utf-8'), settings.client_id_key).decode('utf-8')
