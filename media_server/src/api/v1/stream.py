...
router = APIRouter()
current_dir = FilePath(__file__).parents[3]
...
async def stream_hls_video(
	...
	video_name: Annotated[str, Path(description="Название .m3u8 файла фильма")]
):
	m3u8_file_path = current_dir.joinpath(f"hls/{folder_name}/{video_name}.m3u8")
	if m3u8_file_path.exists():
		return StreamingResponse(
			generate(m3u8_file_path), media_type="application/vnd.apple.mpegurl"
		)
	else:
...
async def stream_hls_segment(
	folder_name: Annotated[str, Path(description="Название папки фильма")],
	segment_name: Annotated[str, Path(description="Название .ts файлов фильма")]
):
